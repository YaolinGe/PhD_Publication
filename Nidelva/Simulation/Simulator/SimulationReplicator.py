...
class SimulationReplicator:
    ...
    def plot_simulation_result(self):
        ...
        ax1.errorbar(np.arange(ibv_2d.shape[1]), np.mean(ibv_2d, axis=0), yerr=np.std(ibv_2d, axis=0), fmt="-o",
                     capsize=5, label="Myopic 2D")
        ax1.errorbar(np.arange(ibv_3d.shape[1]), np.mean(ibv_3d, axis=0), yerr=np.std(ibv_3d, axis=0), fmt="-o",
                     capsize=5, label="Myopic 3D")
        ax1.errorbar(np.arange(ibv_lawnmower.shape[1]), np.mean(ibv_lawnmower, axis=0),
                     yerr=np.std(ibv_lawnmower, axis=0), fmt="-o", capsize=5, label="Lawn mower")
        plt.xlabel('Steps')
        plt.ylabel('IBV')
        ...
        ax2.errorbar(np.arange(rmse_2d.shape[1]), np.mean(rmse_2d, axis=0), yerr=np.std(rmse_2d, axis=0), fmt="-o",
                     capsize=5, label="Myopic 2D")
        ax2.errorbar(np.arange(rmse_3d.shape[1]), np.mean(rmse_3d, axis=0), yerr=np.std(rmse_3d, axis=0), fmt="-o",
                     capsize=5, label="Myopic 3D")
        ax2.errorbar(np.arange(rmse_lawnmower.shape[1]), np.mean(rmse_lawnmower, axis=0),
                     yerr=np.std(rmse_lawnmower, axis=0), fmt="-o", capsize=5, label="Lawn mower")
        plt.xlabel('Steps')
        plt.ylabel('RMSE')
        plt.legend()

        ax3 = fig.add_subplot(gs[1, 0])
        ax3.errorbar(np.arange(ev_2d.shape[1]), np.mean(ev_2d, axis=0), yerr=np.std(ev_2d, axis=0), fmt="-o", capsize=5,
                     label="Myopic 2D")
        ax3.errorbar(np.arange(ev_3d.shape[1]), np.mean(ev_3d, axis=0), yerr=np.std(ev_3d, axis=0), fmt="-o", capsize=5,
                     label="Myopic 3D")
        ax3.errorbar(np.arange(ev_lawnmower.shape[1]), np.mean(ev_lawnmower, axis=0),
                     yerr=np.std(ev_lawnmower, axis=0), fmt="-o", capsize=5, label="Lawn mower")
        plt.xlabel('Steps')
        plt.ylabel('Variance reduction')
        plt.legend()

        ax4 = fig.add_subplot(gs[1, 1])
        ax4.errorbar(np.arange(dist_2d.shape[1]), np.mean(dist_2d, axis=0), yerr=np.std(dist_2d, axis=0), fmt="-o",
                     capsize=5, label="Myopic 2D")
        ax4.errorbar(np.arange(dist_3d.shape[1]), np.mean(dist_3d, axis=0), yerr=np.std(dist_3d, axis=0), fmt="-o",
                     capsize=5, label="Myopic 3D")
        ax4.errorbar(np.arange(dist_lawnmower.shape[1]), np.mean(dist_lawnmower, axis=0),
                     yerr=np.std(dist_lawnmower, axis=0), fmt="-o", capsize=5, label="Lawn mower")
        ...
        plt.legend()
        plt.savefig("/Users/yaoling/OneDrive - NTNU/MASCOT_PhD/Publication/Nidelva/fig/Simulation/Result.pdf")
        plt.close("all")
        pass
