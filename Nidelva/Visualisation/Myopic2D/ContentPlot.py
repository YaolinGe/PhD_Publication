...
class ContentPlot:
    ...
    def __init__(self, knowledge=None, yoyo=None, vmin=28, vmax=28, filename="mean", html=False):
    ...
    def plot(self):
        ...
        # == YOYO ==
        if self.path_yoyo:
            self.path_yoyo = np.array(self.path_yoyo)
            fig.add_trace(go.Scatter3d(
                # print(trajectory),
                x=self.path_yoyo[:, 1],
                y=self.path_yoyo[:, 0],
        ...
        fig.update_layout(
            title={
                'text': "Adaptive 2D myopic illustration",
                'y': 0.9,
                'x': 0.5,
                'xanchor': 'center',
                'yanchor': 'top',
